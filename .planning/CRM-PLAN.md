...
| 2026-04-27 / 28 | **Canonical claim vocabulary locked across the CRM.** Five required column names: `file_number` (internal CCS-generated, FL-NNNNN-YYYY), `claim_number` (carrier-assigned), `policy_number`, `client_name` (the insured), `loss_address`. No synonyms in code. No per-table renames. **Pair rule:** every column on `onboarding_clients` should be on the other CRM spokes when applicable, with the same name and type. | Frank: "everything that's on your onboarding KPI needs to be the same in each of these systems when you add something." `claim_health_records.claim_id` text was a long-standing misnamed `file_number` — kind of drift that compounds into special cases in app code, RLS, and JOINs. See memory entries `feedback_canonical_vocabulary.md` and `feedback_cross_spoke_consistency.md`. |
| 2026-04-27 / 28 | **All 7 CRM spokes now carry `file_number text` directly.** Renamed `claim_health_records.claim_id` → `file_number`; added `file_number` columns to `pa_settlements`, `mediations`, `appraisals` (was JOINing through `litigation_file_id` for everything). Captured in `supabase/migrations/20260428_canonical_file_number_sweep.sql`. | First pass of the canonical-vocabulary sweep — file_number only. The other 4 canonical columns (`claim_number` / `policy_number` / `client_name` / `loss_address`) have known gaps on `pa_settlements` / `mediations` / `appraisals` and on `litigation_files` (missing `claim_number`); those gaps will be closed in subsequent migrations. Backfill of the new empty file_number columns from `litigation_files` via `litigation_file_id` pairs with Step 5. |
| 2026-04-28 | **Canonical CRM Spoke Standard locked.** Two categories required on every spoke (current 7 + all future, expected to grow to 15-18): **IDENTIFIERS** (`file_number`, `claim_number`, `policy_number`, `client_name`, `loss_address`) + **CHARACTERISTICS** (`peril`, `peril_other`, `severity` int 1-5 with CHECK, `status` deferred). New spokes must include all 9 canonical columns in their initial CREATE TABLE migration — no follow-up adds, no exceptions. | Frank: "if I accidentally forget to say these are minimum standards, the MD should speak of this... at very least these items are absolutely important in everyone." The standard is institutionalized in three places: top section of `HANDOFF.md` ("Canonical CRM Spoke Standard — READ FIRST IF DOING SCHEMA WORK"), memory entry `feedback_canonical_spoke_standard.md`, and this decision-log row. Cross-references to `peril_types` lookup table and the canonical-vocabulary lock from the prior row. |
| 2026-04-28 | **All 7 CRM spokes now carry the full canonical column set.** ALTER TABLE sweep added missing columns across all 7 spokes: `claim_number` and `policy_number` (where missing), `client_name` and `loss_address` (where missing — `estimates.loss_address` and others), `peril` text (mediations / appraisals / claim_health_records), `peril_other` text (every spoke), `severity` int 1-5 (every spoke that lacked it). PA Settlements `claim_severity` text → renamed to `severity` and retyped to int 1-5 with the canonical CHECK. Existing peril CHECK constraints dropped from spokes that had them (peril is now sourced from `peril_types` lookup, not enforced at the column level). | Verification matrix confirms 100% compliance across all 7 spokes on all 8 canonical columns (status deferred). DDL applied via Supabase dashboard chunk-by-chunk; capture in a migration file is pending tonight's broader code/UI sweep. |
